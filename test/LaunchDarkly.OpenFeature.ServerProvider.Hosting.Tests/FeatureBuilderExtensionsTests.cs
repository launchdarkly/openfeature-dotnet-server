...
namespace LaunchDarkly.OpenFeature.ServerProvider.Hosting.Tests
{
    public class FeatureBuilderExtensionsTests
    {
        ...
        [Fact]
        public async Task ResolvesFlagsUsingOptionsConfiguredOutsideOfTheBuilder()
        {
            var testData = FlagData("enabled-flag", true);

            var host = new HostBuilder().ConfigureServices(services =>
            {
                services.Configure<LaunchDarklyProviderOptions>(options =>
                {
                    options.SdkKey = "fake-key";
                    options.ConfigureSdk = config => ConfigureOffline(testData, config);
                });
        ...
        public async Task SupportsDomainScopedProviders()
        {
            ...
            using (var scope = host.Services.CreateScope())
            {
                ...
                var betaProvider = (Provider)scope.ServiceProvider.GetRequiredKeyedService<FeatureProvider>("beta");
                Assert.Same(betaProvider.GetClient(),
                    scope.ServiceProvider.GetRequiredKeyedService<ILdClient>("beta"));
            }
